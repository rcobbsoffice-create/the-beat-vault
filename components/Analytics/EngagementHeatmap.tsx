  ...
  useEffect(() => {
    ...
    data.forEach(p => {
      if (p.isDropoff || p.isPeak) {
        ...
        ctx.fillStyle = p.isPeak ? '#10B981' : '#EF4444';
        ctx.fill();
        ctx.strokeStyle = '#000';
        ctx.lineWidth = 1;
        ctx.stroke();
    ...
    <Card className="p-6 bg-dark-950 border-white/5 relative overflow-hidden group">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            {title}
            <Badge variant="outline" className="text-[10px] border-emerald-500/30 text-emerald-500 uppercase tracking-widest font-bold bg-emerald-500/5">
        ...
        <div className="flex gap-2">
           <div className="flex items-center gap-1.5 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
              ...
              <span className="text-[10px] text-emerald-500 font-bold">92% Retention</span>
           </div>
        </div>
      </div>
      ...
      <div className="relative h-48 w-full mt-4 bg-black/40 rounded-2xl border border-white/5 overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
           <div className="absolute left-[35%] top-[10%] p-2 bg-dark-900 border border-emerald-500/50 rounded-lg shadow-2xl z-20">
              <p className="text-[9px] text-emerald-500 font-black uppercase mb-1">Peak Engagement</p>
      ...
      <div className="flex justify-between mt-4 px-1">
        ...
        <div className="flex gap-4">
           <div className="flex items-center gap-1 text-[9px] text-gray-400 uppercase tracking-widest font-bold">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Hook
           </div>
           <div className="flex items-center gap-1 text-[9px] text-gray-400 uppercase tracking-widest font-bold">
              <div className="w-1.5 h-1.5 rounded-full bg-red-500" /> Dropoff
           </div>
        </div>
