...
export default function BeatUploadPage() {
  ...
  const handleAIAnalyze = async () => {
    ...
    const result = await analyzeWithUI({
      ...
      forceNewArtwork: true, // Always force a new generation on manual trigger
      currentArtworkPreview: artworkPreview,
    });

    if (result) {
      if (result.title) setTitle(result.title);
      if (result.primary_genre && Array.isArray(availableGenres)) {
        const match = availableGenres.find(g => g && g.toLowerCase() === result.primary_genre!.toLowerCase());
        if (match) setPrimaryGenre(match);
        else { setShowCustomGenre(true); setCustomGenre(result.primary_genre!); }
      }
      if (result.secondary_genre && Array.isArray(availableGenres)) {
        const match = availableGenres.find(g => g && g.toLowerCase() === result.secondary_genre!.toLowerCase());
        if (match) setSecondaryGenre(match);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 4. Upload Artwork if present (Manual or AI generated)
      let artworkUrl = null;
      if (artworkFile || (artworkPreview && artworkPreview.startsWith('http'))) {
        const artUri = artworkFile ? artworkFile.uri : artworkPreview;
        const artResponse = await fetch(artUri!);
      ...
      // 6. Trigger Audio Fingerprinting (async)
        ...
        <View className="relative z-10">
          ...
          <View className="flex-row items-start justify-between">
            ...
            >
              ...
              <Text className="text-gray-300 font-black text-[10px] uppercase tracking-[0.2em]"><Text>Switch </Text><Text className="text-primary">Bulk</Text></Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
        ...
        <View className="space-y-8">
          ...
          >
            ...
            <Text className="text-white font-black text-2xl uppercase italic tracking-tighter text-center">
              {audioFile ? audioFile.name : (
                <><Text>SELECT </Text><Text className="text-primary">MASTER</Text><Text> TRACK</Text></>
              )}
            </Text>
          ...
          <View className="flex-col lg:flex-row gap-8">
             ...
             <TouchableOpacity 
                ...
                className="rounded-[40px] bg-dark-900/50 border border-white/10 overflow-hidden items-center justify-center relative shadow-2xl"
              >
                {artworkPreview ? (
                  <Image 
                    source={{ uri: artworkPreview }} 
                    style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, width: 256, height: 256, backgroundColor: '#111' }}
                    resizeMode="cover"
                    onError={() => setArtworkPreview(null)}
                  />
                ) : (
             ...
             <View className="flex-1 space-y-8">
                 <View className="mb-2">
                     <Button
                        ...
                        className="w-full py-4 bg-purple-600/20 border border-purple-500/30 rounded-2xl"
                      >
                        {isAnalyzing ? (
                          <View className="flex-row items-center gap-2">
                             <ActivityIndicator color="#A855F7" size="small" />
                             <Text className="text-purple-400 font-bold uppercase tracking-widest text-xs">Analyzing Audio...</Text>
                          </View>
                        ) : (
                          <View className="flex-row items-center gap-2">
                            <Text className="text-purple-400 font-bold uppercase tracking-widest text-xs">✨ Auto-Fill with AI</Text>
                          </View>
                        )}
                      </Button>
                 </View>
          ...
          <View className="relative p-10 bg-dark-900/60 rounded-[40px] border border-white/5 overflow-hidden">
             ...
             <View className="space-y-10 relative z-10">
               ...
               <View>
                ...
                {/* Watermark Toggle */}
                <Card className="p-6 bg-primary/5 border border-primary/20 rounded-[30px]">
                   <View className="flex-row items-center justify-between">
                      <View className="flex-1 mr-4">
                         <Text className="text-white font-bold text-lg mb-1">Watermark Preview</Text>
                         <Text className="text-gray-500 text-xs">A vocal tag will be repeated every 10s to protect your track.</Text>
                      </View>
                      <TouchableOpacity 
                        onPress={() => setWatermarkEnabled(!watermarkEnabled)}
                        className={`w-14 h-8 rounded-full justify-center px-1 ${watermarkEnabled ? 'bg-primary items-end' : 'bg-dark-800 items-start'}`}
                      >
