...
export default function BulkUploadPage() {
  ...
  const analyzeOneBeat = async (beatItem: BulkBeatItem) => {
    setAnalyzingId(beatItem.id);
    updateBeat(beatItem.id, { aiStatus: 'analyzing' });
    try {
      const result = await analyze({
        audioUri: beatItem.file.uri,
        mimeType: beatItem.file.mimeType || 'audio/wav',
        generateArtwork: true,
        forceNewArtwork: true,
      });
      updateBeat(beatItem.id, {
        title: result.title || beatItem.title,
        genres: [result.primary_genre, result.secondary_genre].filter(Boolean) as string[],
        bpm: result.bpm?.toString() || beatItem.bpm,
        key: result.key || '',
        tags: result.mood_tags?.join(', ') || '',
        description: result.description || '',
        artwork_url: result.artworkUrl,
        aiStatus: 'done',
      });
    } catch (err: any) {
      console.error(`[AI Bulk] Error analyzing ${beatItem.title}:`, err.message);
      Alert.alert('AI Error', `Failed to analyze "${beatItem.title}": ${err.message}`);
      updateBeat(beatItem.id, { aiStatus: 'idle' });
    } finally {
      setAnalyzingId(null);
    }
  };

  const analyzeAllBeats = async () => {
    const pending = beats.filter(b => b.aiStatus !== 'done');
    if (pending.length === 0) {
      Alert.alert('Done', 'All beats already analyzed.');
      return;
    }
    setIsAnalyzingAll(true);
    Alert.alert('AI Batch Analysis', `Analyzing ${pending.length} tracks. This may take a minute...`, [], { cancelable: true });
    for (const beatItem of pending) {
      await analyzeOneBeat(beatItem);
    }
    setIsAnalyzingAll(false);
    Alert.alert('Complete', `AI analysis finished for ${pending.length} tracks!`);
  };
      ...
      try {
        ...
        const { data: beat, error: beatError } = await supabase
          ...
          .insert({
            ...
            key: beatItem.key || null,
            description: beatItem.description || null,
            mood_tags: beatItem.tags ? beatItem.tags.split(',').map(t => t.trim()).filter(Boolean) : [],
            artwork_url: beatItem.artwork_url || null,
            status: 'draft',
            audio_url: 'pending',
            preview_url: 'pending'
          })
          ...
          <View className="flex-row gap-2">
            {beats.length > 0 && (
              <Button 
                ...
                onPress={analyzeAllBeats} 
                disabled={isUploading || isAnalyzingAll}
                className="border-purple-500/30 bg-purple-600/10"
              >
                {isAnalyzingAll ? (
                  <View className="flex-row items-center gap-1">
                    <ActivityIndicator color="#A855F7" size="small" />
                    <Text className="text-purple-400 text-[10px] font-bold">Analyzing...</Text>
                  </View>
                ) : (
                  <Text className="text-purple-400 text-[10px] font-bold">✨ Analyze All</Text>
                )}
              </Button>
            )}
          ...
          <View className="gap-4">
            {beats.map((item) => (
              <Card key={item.id} className="p-4 bg-white dark:bg-dark-900 border-slate-200 dark:border-white/5">
                <View className="flex-row gap-4">
                  <View className="w-12 h-12 bg-slate-100 dark:bg-dark-800 rounded-lg overflow-hidden items-center justify-center">
                    {item.artwork_url ? (
                      <Image source={{ uri: item.artwork_url }} className="w-full h-full" />
                    ) : (
                  ...
                  <View className="flex-1">
                    <View className="flex-row items-center justify-between mb-2">
                       <TextInput 
                          value={item.title} 
                          onChangeText={(val: string) => updateBeat(item.id, { title: val })}
                          className="text-slate-900 dark:text-white font-bold text-base flex-1"
                       />
                       <View className="flex-row items-center gap-2">
                         <TouchableOpacity 
                           onPress={() => analyzeOneBeat(item)} 
                           disabled={isUploading || analyzingId === item.id}
                           className="px-2 py-1 bg-purple-600/10 border border-purple-500/30 rounded-lg"
                         >
                           {analyzingId === item.id ? (
                             <ActivityIndicator color="#A855F7" size="small" />
                           ) : (
                             <Text className="text-purple-400 text-[10px] font-bold">{item.aiStatus === 'done' ? '✅' : '✨'}</Text>
                           )}
                         </TouchableOpacity>
