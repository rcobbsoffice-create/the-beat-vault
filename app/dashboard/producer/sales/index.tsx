...
export default function ProducerSalesPage() {
  ...
  const fetchSalesData = async () => {
    ...
    setLoading(true);
    try {
      // Workaround for missing purchases.producer_id column:
      // 1. Fetch all beat IDs belonging to this producer
      ...
      if (beatIds.length === 0) {
        setTransactions([]);
        setStats({ totalRevenue: 0, totalSales: 0, pendingPayout: 0, avgSaleValue: 0 });
        setLoading(false);
        return;
      }

      // 2. Fetch purchases manually filtering by those beat IDs
