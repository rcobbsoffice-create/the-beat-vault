    ...
    <ScrollView className="flex-1 bg-dark-950 p-6">
      ...
      <View className="flex-row flex-wrap gap-4 mb-8">
         {[
           ...
           { label: 'Platform Revenue', value: `$${totalRevenue.toLocaleString()}`, sub: 'Estimated Real-time', Icon: DollarSign, color: '#60A5FA' },
           { label: 'Active DSPs', value: '18', sub: 'Worldwide Distribution', Icon: Globe, color: '#93C5FD' },
           { label: 'Conversion Rate', value: '3.4%', sub: 'Streams to Purchases', Icon: Target, color: '#9CA3AF' },
         ].map((stat, i) => (
           <Card key={i} className="p-6 bg-dark-900/80 border-white/5 flex-1 min-w-[160px]">
      ...
      <View className="gap-8">
        {/* DSP Breakdown */}
        <Card className="p-6 bg-dark-900/80 border-white/5">
              ...
              {data.map((dsp, i) => (
                <View key={i} className="gap-2">
                   <View className="flex-row items-center justify-between">
                      ...
                      <Text className="text-sm font-black italic text-blue-400">+${dsp.revenue_usd}</Text>
                   </View>
        ...
        <View className="flex-col md:flex-row gap-4">
           ...
           <Card className="p-6 border-blue-500/20 bg-blue-500/5 flex-1">
              <Text className="text-sm font-black uppercase italic tracking-widest text-blue-400 mb-2 flex-row items-center gap-2">
                <TrendingUp size={16} color="#60A5FA" /> Growth Path
              </Text>
              <Text className="text-xs text-gray-400 font-medium leading-relaxed">
