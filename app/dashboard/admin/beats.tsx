...
export default function AdminBeatsPage() {
  ...
  const { analyzeWithUI, isAnalyzing: isAnalyzingModal } = useAIAnalyze();
  ...
  const handleModalAIAnalyze = async () => {
    const audioUri = audioFile?.uri || form.audio_url;
    if (!audioUri || audioUri === 'pending') {
      Alert.alert('Error', 'Please select an audio file first, or ensure the beat has an existing audio URL.');
      return;
    }
    ...
    const result = await analyzeWithUI({
      ...
      generateArtwork: true, // Always allow new AI artwork on auto-fill
      forceNewArtwork: true, // Force new generation even if one exists
      currentArtworkPreview: null, // Force new generation even if one exists
    });

    if (result) {
      console.log('[AI Modal Debug] Analysis Result:', result);
      
      setForm(prev => {
        const next = { ...prev };
        
        if (result.title) next.title = result.title;
        if (result.bpm) next.bpm = result.bpm.toString();
        
        // Key mapping
        if (result.key) {
           const match = KEYS.find(k => 
             k.toLowerCase() === result.key!.toLowerCase() || 
             k.toLowerCase().includes(result.key!.toLowerCase()) ||
             result.key!.toLowerCase().includes(k.toLowerCase())
           );
           next.key = match || result.key;
        }

        // Genre mapping
        if (result.primary_genre) {
          next.genre = result.primary_genre;
          
          // Fuzzy match against primary DNA badges
          const primaryMatch = GENRES.find(g => 
            g.toLowerCase() === result.primary_genre!.toLowerCase() ||
            result.primary_genre!.toLowerCase().includes(g.toLowerCase())
          );
          
          const secondaryMatch = result.secondary_genre ? GENRES.find(g => 
            g.toLowerCase() === result.secondary_genre!.toLowerCase() ||
            result.secondary_genre!.toLowerCase().includes(g.toLowerCase())
          ) : null;

          const newGenres = [primaryMatch, secondaryMatch].filter(Boolean) as string[];
          if (newGenres.length > 0) {
            next.genres = newGenres;
          } else {
            // Fallback to literal AI genres if no DNA match
            next.genres = [result.primary_genre, result.secondary_genre].filter(Boolean) as string[];
          }
        }

        if (result.mood_tags) next.mood_tags = result.mood_tags;
        if (result.description) next.description = result.description;
        if (result.artworkUrl) {
          next.artwork_url = result.artworkUrl;
          setArtworkFile(null); // Clear manual selection to show the new AI artwork
        }
        
        console.log('[AI Modal Debug] New Form State:', next);
        return next;
      });
    }
  };
              ...
              <ScrollView className="flex-1 p-6">
                 <View className="gap-6">
                    ...
                    <View className="gap-4">
                       ...
                       {/* AI Auto-Fill Button */}
                       <TouchableOpacity
                         onPress={handleModalAIAnalyze}
                         disabled={isAnalyzingModal || (!audioFile && !form.audio_url)}
                         className="w-full py-3 bg-purple-600/20 border border-purple-500/30 rounded-xl items-center justify-center mb-2"
                       >
                         {isAnalyzingModal ? (
                           <View className="flex-row items-center gap-2">
