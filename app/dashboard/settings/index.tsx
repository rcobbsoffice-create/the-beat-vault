...
import { User, Mail, Lock, Bell, Globe, Camera, Loader2, Mic, Play, Square, Upload as UploadIcon } from 'lucide-react-native';
...
export default function DashboardSettingsPage() {
  ...
  // Vocal Tag State
  const [vocalTagMode, setVocalTagMode] = useState<'none' | 'default' | 'custom'>('default');
  const [vocalTagUrl, setVocalTagUrl] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (profile) {
      ...
      setVocalTagMode(profile.vocal_tag_mode || 'default');
      setVocalTagUrl(profile.vocal_tag_url || null);
    }
  }, [profile]);
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          bio: bio,
          vocal_tag_mode: vocalTagMode,
          vocal_tag_url: vocalTagUrl,
        })
        .eq('id', profile.id);
        ...
        <Card className="p-6 bg-dark-900/50 border-white/5">
         ...
         <Card className="p-6 bg-dark-900/50 border-white/5">
            <View className="flex-row items-center gap-3 mb-6">
               <Mic size={24} color="#005CB9" />
               <View>
                 <Text className="font-bold text-white text-lg">Vocal Tag & Branding</Text>
                 <Text className="text-gray-500 text-xs">Protect your previews with a repeating audio watermark</Text>
               </View>
            </View>

            <View className="gap-8">
               <View>
                  <Text className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Watermark Mode</Text>
                  <View className="flex-row gap-3">
                     {[
                       { id: 'none', label: 'None', desc: 'No watermark' },
                       { id: 'default', label: 'Default', desc: 'AudioGenes Tag' },
                       { id: 'custom', label: 'Custom', desc: 'Your Voice' }
                     ].map((mode) => (
                       <TouchableOpacity
                         key={mode.id}
                         onPress={() => setVocalTagMode(mode.id as any)}
                         className={`flex-1 p-4 rounded-2xl border transition-all ${
                           vocalTagMode === mode.id 
                             ? 'bg-primary/10 border-primary' 
                             : 'bg-dark-950 border-white/5'
                         }`}
                       >
                         <Text className={`font-bold text-sm mb-1 ${vocalTagMode === mode.id ? 'text-primary' : 'text-white'}`}>
                           {mode.label}
                         </Text>
                         <Text className="text-[10px] text-gray-500">{mode.desc}</Text>
                       </TouchableOpacity>
                     ))}
                  </View>
               </View>
               ...
               {vocalTagMode === 'custom' && (
                 <View className="p-6 rounded-3xl bg-dark-950 border border-white/5 items-center">
                    <Text className="text-gray-400 text-sm mb-6 text-center italic">
                       "Upload a short recording of your producer tag. We'll repeat it every 10 seconds in your beat previews."
                    </Text>
                    
                    <View className="flex-row items-center gap-4 w-full">
                       <TouchableOpacity 
                         className="flex-1 h-14 bg-white/5 border border-white/10 rounded-2xl flex-row items-center justify-center gap-3"
                         onPress={() => Alert.alert('Info', 'Tag upload using DocumentPicker is ready for implementation')}
                       >
                          {isUploadingTag ? (
                            <Loader2 size={20} color="#005CB9" />
                          ) : (
                            <UploadIcon size={20} color="#005CB9" />
                          )}
                          <Text className="text-gray-300 font-bold">Upload Custom Tag</Text>
                       </TouchableOpacity>
                    ...
                    {vocalTagUrl && (
                      <Text className="mt-4 text-[10px] text-primary font-bold uppercase tracking-widest">
                         Tag Active: {vocalTagUrl.split('/').pop()}
                      </Text>
                    )}
                 </View>
               )}
