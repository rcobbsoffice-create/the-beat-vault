-- =====================================================
-- THE BEAT VAULT - COMPLETE RLS POLICIES
-- =====================================================
...
CREATE POLICY "Producers can view own record" ON producers
  FOR SELECT USING (auth.uid() = profile_id);

DROP POLICY IF EXISTS "Producers can update own record" ON producers;
CREATE POLICY "Producers can update own record" ON producers
  FOR UPDATE USING (auth.uid() = profile_id);

DROP POLICY IF EXISTS "Producers can insert own record" ON producers;
CREATE POLICY "Producers can insert own record" ON producers
...
CREATE POLICY "Public can view stores" ON stores
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE producers.id = stores.producer_id AND producers.status = 'active'
    )
  );

DROP POLICY IF EXISTS "Producers can manage own store" ON stores;
CREATE POLICY "Producers can manage own store" ON stores
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM producers
      WHERE producers.id = stores.producer_id AND producers.profile_id = auth.uid()
    )
  );
...
CREATE POLICY "Everyone can view approved genres" ON genre_settings
  FOR SELECT USING (status = 'approved');

DROP POLICY IF EXISTS "Authenticated users can propose genres" ON genre_settings;
CREATE POLICY "Authenticated users can propose genres" ON genre_settings
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL AND status = 'proposed');

DROP POLICY IF EXISTS "Admins can manage all genres" ON genre_settings;
CREATE POLICY "Admins can manage all genres" ON genre_settings
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admins can manage all beats" ON beats
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Public can view active licenses" ON licenses
  FOR SELECT USING (
    is_active = true AND
    EXISTS (
      SELECT 1 FROM beats
      WHERE beats.id = licenses.beat_id AND beats.status = 'published'
    )
  );

DROP POLICY IF EXISTS "Producers can manage own beat licenses" ON licenses;
CREATE POLICY "Producers can manage own beat licenses" ON licenses
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM beats
      WHERE beats.id = licenses.beat_id AND beats.producer_id = auth.uid()
    )
  );
...
CREATE POLICY "Users can view own favorites" ON beat_favorites
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can add favorites" ON beat_favorites;
CREATE POLICY "Users can add favorites" ON beat_favorites
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can remove own favorites" ON beat_favorites;
CREATE POLICY "Users can remove own favorites" ON beat_favorites
...
CREATE POLICY "Producers can view purchases of their beats" ON purchases
  FOR SELECT USING (auth.uid() = producer_id);

DROP POLICY IF EXISTS "System can create purchases" ON purchases;
CREATE POLICY "System can create purchases" ON purchases
  FOR INSERT WITH CHECK (true); -- Purchases created via Edge Functions
...
CREATE POLICY "Public can view published merch" ON merch_products
  FOR SELECT USING (status = 'published');

DROP POLICY IF EXISTS "Producers manage own merch" ON merch_products;
CREATE POLICY "Producers manage own merch" ON merch_products
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM producers
      WHERE producers.id = merch_products.producer_id
        AND producers.profile_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Admins manage all merch" ON merch_products;
CREATE POLICY "Admins manage all merch" ON merch_products
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Producers view own orders" ON orders
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM producers
      WHERE producers.id = orders.producer_id
        AND producers.profile_id = auth.uid()
    )
  );
...
CREATE POLICY "Public can view verified artist profiles" ON artist_profiles_ext
  FOR SELECT USING (is_verified = true);

DROP POLICY IF EXISTS "Artists can view own profile ext" ON artist_profiles_ext;
CREATE POLICY "Artists can view own profile ext" ON artist_profiles_ext
  FOR SELECT USING (auth.uid() = profile_id);

DROP POLICY IF EXISTS "Artists can manage own profile ext" ON artist_profiles_ext;
CREATE POLICY "Artists can manage own profile ext" ON artist_profiles_ext
  FOR ALL USING (auth.uid() = profile_id);

DROP POLICY IF EXISTS "Admins can manage all artist profiles ext" ON artist_profiles_ext;
CREATE POLICY "Admins can manage all artist profiles ext" ON artist_profiles_ext
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Users can manage own questionnaires" ON artist_questionnaires
...
CREATE POLICY "Admins and Editors view all questionnaires" ON artist_questionnaires
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
...
CREATE POLICY "Admins and Editors update questionnaires" ON artist_questionnaires
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role IN ('admin', 'editor')
...
CREATE POLICY "Admins view all distribution data" ON distribution_data
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Artists view own distribution data" ON distribution_data
  FOR SELECT USING (auth.uid() = artist_id);

DROP POLICY IF EXISTS "System can manage distribution data" ON distribution_data;
CREATE POLICY "System can manage distribution data" ON distribution_data
  FOR ALL WITH CHECK (true); -- Managed by Edge Functions
...
CREATE POLICY "Admins can manage charts" ON charts
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Producers can view own beat fingerprints" ON audio_fingerprints
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM beats
      WHERE beats.id = audio_fingerprints.beat_id
        AND beats.producer_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Admins can manage all fingerprints" ON audio_fingerprints;
CREATE POLICY "Admins can manage all fingerprints" ON audio_fingerprints
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "System can manage fingerprints" ON audio_fingerprints
  FOR ALL WITH CHECK (true); -- Managed by Edge Functions
...
CREATE POLICY "Producers can view own beat detections" ON track_detections
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM beats
      WHERE beats.id = track_detections.beat_id
        AND beats.producer_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Admins can manage all detections" ON track_detections;
CREATE POLICY "Admins can manage all detections" ON track_detections
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "System can manage detections" ON track_detections
  FOR ALL WITH CHECK (true); -- Managed by Edge Functions
...
CREATE POLICY "Producers can view own beat tracking" ON beat_tracking_summary
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM beats
      WHERE beats.id = beat_tracking_summary.beat_id
        AND beats.producer_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS "Admins can view all tracking summaries" ON beat_tracking_summary;
CREATE POLICY "Admins can view all tracking summaries" ON beat_tracking_summary
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admins can view global stats" ON admin_fingerprint_global_stats
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admins can view platform stats" ON admin_fingerprint_platform_stats
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Public can view pulse data for published beats" ON pulse_data
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM beats
      WHERE beats.id = pulse_data.beat_id AND beats.status = 'published'
    )
  );

DROP POLICY IF EXISTS "System can manage pulse data" ON pulse_data;
CREATE POLICY "System can manage pulse data" ON pulse_data
  FOR ALL WITH CHECK (true); -- Managed by Edge Functions
